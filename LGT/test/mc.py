...
def calc_teq(bare_arg, O, init_lat, mcstep=metropolis, stride=10, tol=1e-3, max_step=500, verbose=0, fig_dir=None, use_lat=True):
	...
	for i in range(max_step):
		...
		if i > 2*stride:
			diff = np.mean(O_cold[i-stride:i] - np.mean(O_cold[i-stride:i]))
...
def calc_tac(bare_arg, O, init_lat, mcstep=metropolis, t_eq=100, n_conf_ac=500, verbose=False, fig_dir=None, use_lat=False):
...
def calc_teq_tac(bare_arg, O, init_lat, mcstep=metropolis, max_steps=500, tol=1e-5, verbose=False, fig_dir=None, use_lat=False):
	...
	for i in range(max_steps):
		ac_hist[i] = autocorrelation(O_hist, i)
